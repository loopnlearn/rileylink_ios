...
class PodStateTests: XCTestCase {
    ...
    func testNonceValues() {
        var podState = PodState(address: 0x1f000000, pmVersion: "1.1.0", piVersion: "1.1.0", lot: 42560, tid: 661771)
    ...
    func testResyncNonce() {
        do {
            ...
            var podState = PodState(address: config.address, pmVersion: config.firmwareVersion.description, piVersion: config.iFirmwareVersion.description, lot: config.lot, tid: config.tid)
